...
{   //FIXME
    ...
    Deque* dqstack = dequeNew();
    dequePushFront(dqstack, source);
    while(dequeIsEmpty(dqstack) != 1)
    {
        Vertex* curelement = dequeFront(dqstack);
        dequePopFront(dqstack);
        if( curelement->isVisited == 0)
        {
            curelement->isVisited = 1;
            int i;
            for(i = 0; i<curelement->numNeighbors; i++)
            {
                //printf(" i = %i\n", i);
                if(curelement->neighbors[i] == destination)
                {
                    //printf("Element found\n");
                    dequeDelete(dqstack);
                    return 1;
                }
                dequePushFront(dqstack, curelement->neighbors[i]);
            }
        }
    }
    dequeDelete(dqstack);
    ...
    while(dequeIsEmpty(dqstack) != 1)
    {
        ...
        if( curelement->isVisited == 0)
        {
            ...
            for(i = 0; i<curelement->numNeighbors; i++)
            {
                ...
                dequePushBack(dqstack, curelement->neighbors[i]);
            }
        }
    }
    dequeDelete(dqstack);
